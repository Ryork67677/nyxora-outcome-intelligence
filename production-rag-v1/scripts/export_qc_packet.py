#!/usr/bin/env python3
"""GOLD-001: build the owner QC packet for a reviewed batch.
...
The packet exists so a person can decide, so it does not decide: the decisions file it
writes carries ``decision: null`` for every candidate, and a repaired candidate also
carries ``approves_evidence_hash: null``, so accepting a repair means quoting the hash of
the evidence being accepted rather than trusting that a repair happened.

Three states are kept apart throughout, because collapsing them is the failure this
pipeline exists to prevent: ``precheck_holdout_ready`` (structural, decided by a script),
``human_verified`` (owner approval, decided only by a person), and ``holdout_eligible``
(derived from both plus deterministic claim support).
"""
...
def compose(batch: dict, repairs: dict) -> list[dict]:
    ...
    for record in batch["records"]:
        ...
        changed = bool(final.get("revisions") or final.get("anchor_revisions"))
        out.append({
            "record": final,
            "generated": record,
            "review": repairs["review"][candidate_id],
            "was_repaired": changed,
        })
    return out
...
def render(packet: dict) -> str:
    ...
    number = packet["batch"]
    lines = [
        f"# GOLD-001 — batch {number:03d} owner QC packet",
        "",
        ...
        "",
        ("The statuses below were produced by the authoring model reading its own "
         "output against the frozen evidence. That is a self-check. It is not a second "
         "opinion from an independent party, it is not verification, and it confers no "
         "status on any candidate. A `READY_FOR_OWNER_REVIEW` label means the author "
         "found nothing wrong — which is exactly the claim an independent reviewer is "
         "there to test."),
        "",
        ...
        "| --- | --- | --- |",
        ("| `precheck_holdout_ready` | a script | the record is structurally checkable "
         "— hashes, offsets, critical strings inside their own spans, no critical "
        ...
        "",
        (f"{packet['precheck_holdout_ready']} of {len(entries)} candidates are "
         "`precheck_holdout_ready`. That is not an argument for approving them: the "
         f"review below recommends {counts.get('REJECT_RECOMMENDED', 0)} for rejection "
         f"and repaired {packet['repaired_candidates']}, and every one of those was "
         "precheck-ready before the review looked at it."),
        "",
    ...
    for entry in entries:
        ...
        lines += ["", "### Internal review findings", ""]
        lines += ([f"- {f}" for f in review["findings"]]
                  or ["- No finding. The candidate is as generated."])
        if entry["was_repaired"]:
            ...
            for revision in record.get("revisions", []):
                lines += [f"- **{revision['field']} rewritten** ({revision['reason']})",
                          f"  - was: {revision['from']}",
                          f"  - now: {revision['to']}"]
        ...
        if review["status"] == "REJECT_RECOMMENDED":
            lines += [("The review recommends rejecting this candidate. That "
                       "recommendation is not a decision and does not bind you."), ""]
        lines += ["---", ""]

    defects = packet.get("generator_defects_found") or []
    if defects:
        lines += ["## Generator defects found during this review", "",
...
def main() -> int:
    ...
    packet = {
        ...
        "note": ("A QC packet is a request for decisions, not a record of them. The "
                 "internal review it reports is authoring, not independent "
                 "verification. Nothing here is verified, approved or eligible."),
    ...
    (out_dir / f"gold_batch_{number:03d}_qc.json").write_text(
        json.dumps(packet, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    (out_dir / f"gold_batch_{number:03d}_qc.md").write_text(
    ...
    decisions = {
        ...
        "instructions": (
            ...
            "hash, so a repair cannot be approved by someone who only saw the "
            "original."),
        "decided_by": None,
        "decisions": [
            {
    ...
    (out_dir / f"human_decisions_batch_{number:03d}.json").write_text(
    ...
    print(f"wrote {out_dir}/gold_batch_{number:03d}_qc.md, .json and "
          f"human_decisions_batch_{number:03d}.json")
    return 0
